...
@pytest.mark.asyncio
async def test_filter_removes_gmail_without_connection(db_session, crm_user) -> None:
    full = tools_for_palette_mode("full")
    names_before = {t["function"]["name"] for t in full}
    assert "gmail_list_messages" in names_before
    out = await filter_tools_for_user_connectors(db_session, crm_user.id, full)
...
@pytest.mark.asyncio
async def test_filter_keeps_gmail_when_linked(db_session, crm_user) -> None:
    db_session.add(
        ConnectorConnection(
            user_id=crm_user.id,
...
@pytest.mark.asyncio
async def test_resolve_turn_palette_falls_back_when_too_small(monkeypatch, db_session, crm_user) -> None:
    ...
    pal = await mod.resolve_turn_tool_palette(db_session, crm_user)
    assert len(pal) > 10
