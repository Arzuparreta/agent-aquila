    ...
    async def classify_relevance(
        ...
        *,
        ...
        system = (
            ...
            "- 'actionable' = the user must reply, act, or be alerted within "
            "hours. Examples: a real person asking a question, a booking or "
            "concert inquiry, a contract draft, a meeting request, a payment "
            "or refund issue from a customer, an UNAUTHORIZED account access "
            "warning the user did not trigger.\n"
            "- 'informational' = real updates worth preserving but not "
            "requiring a chat thread (e.g. a partner's monthly recap from a "
            "human, project status updates, shipping notifications for "
            "something the user ordered).\n"
            "- 'noise' = anything automated that confirms an action the user "
            "themselves performed or that nobody would reply to. Examples: "
            "newsletters, digests, marketing, product announcements, "
            "verification / OTP / 2FA codes, password reset confirmations, "
            "'new sign-in to <product>' messages, 'passkey added' / "
            "'passkey removed', 'device added', 'security alert' from a "
            "device the user owns, social-network notifications "
            "(LinkedIn, Twitter/X, Facebook, Reddit, Discord, Notion), "
            "GitHub notifications, calendar invites you already accepted, "
            "subscription renewals, generic 'welcome to ...' onboarding.\n"
            "\n"
            "Routine security / account notifications are noise by default. "
            "Only label them 'actionable' when the wording explicitly says "
            "the activity was unauthorized, suspicious, or asks the user to "
            "secure the account because something was compromised.\n"
            "\n"
            "When uncertain between actionable and informational, prefer "
            "'informational'. When uncertain between informational and noise "
            "for an automated/transactional sender, prefer 'noise'."
        )
        user_msg = (
